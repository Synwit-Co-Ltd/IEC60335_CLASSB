...
;*******************************************************************************
; Function Name  : SelfTest_StartUpCPUTest
; Description    : Full Cortex-M4 CPU test at start-up
...
SelfTest_StartUpCPUTest PROC
    EXPORT SelfTest_StartUpCPUTest
    MOVS R0, #0x00000000      ; Set Z(ero) Flag
    BNE.W FailSafeHandle         ; Fails if Z clear
    SUBS R0,#1                ; Set N(egative) Flag
    BPL.W FailSafeHandle         ; Fails if N clear
    ADDS R0,#2                ; Set C(arry) Flag and do not set Z
    BCC.W FailSafeHandle         ; Fails if C clear
    MOVS R0, #0x80000000      ; Prepares Overflow test
    ADDS R0, R0, R0           ; Set V(overflow) Flag
    BVC.W FailSafeHandle         ; Fails if V clear
    ...
    BNE.W FailSafeHandle         ; Fails if Q+N+C=V clear
