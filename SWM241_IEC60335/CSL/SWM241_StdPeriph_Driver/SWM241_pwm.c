...
void PWM_Init(uint32_t pwmx, PWM_InitStructure * initStruct)
{
	...
	PWMx->MODE = ((initStruct->PWMnXN ? 5 : 0) << PWM_MODE_MODE_Pos) |
				 (1 << PWM_MODE_CLKSRC_Pos) |
				 ((initStruct->clkdiv - 1) << PWM_MODE_CLKDIV_Pos);
	...
	if((pwmx%2) == 0)
	{
		...
		case((uint32_t)PWM1):
			...
			if(initStruct->HEndIE | initStruct->NCycleIE | initStruct->HCycleIE) NVIC_EnableIRQ(PWM1_GPIOA_IRQn);
			break;
		...
		case((uint32_t)PWM2):
			...
			if(initStruct->HEndIE | initStruct->NCycleIE | initStruct->HCycleIE) NVIC_EnableIRQ(PWM2_GPIOB_IRQn);
			break;
		...
		case((uint32_t)PWM3):
			...
			if(initStruct->HEndIE | initStruct->NCycleIE | initStruct->HCycleIE) NVIC_EnableIRQ(PWM3_GPIOC_SAFETY_IRQn);
			break;
		}
	}
	else
